...
const Page = () => {
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      const percentageUsed = initializeChart("luwa1", values, 250);
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      const percentageUsed = initializeChart("luwa2", values, 250);
  ...
  useEffect(() => {
    if (data.length > 0) {
      const values = data.map((item) => item.luwa_3A_kW); // Updated to LUWA 2 kW
      const percentageUsed = initializeChart("luwa3A", values, 250);
      setPercentageUsedDataL2(percentageUsed);
    }
  }, [data]);
  ...
  useEffect(() => {
    if (data.length > 0) {
      const values = data.map((item) => item.luwa_3B_kW); // Updated to LUWA 2 kW
      const percentageUsed = initializeChart("luwa3B", values, 250);
      setPercentageUsedDataL2(percentageUsed);
    }
  }, [data]);
  ...
  return (
    <div className="p-5">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="p-0">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-4">
            <CardTitle className="text-xl font-bold">LUWA 3A</CardTitle>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="none"
          ...
          <CardContent className="flex justify-evenly">
            ...
            >
              ...
              >
                {data.map((item) => item.luwa_3A_kW.toFixed(0))} kW
              </div>
              <canvas id="luwa3A" width="100" height="100" />
            </div>
            <div className="">
              {data.map((item) => {
                return (
                  <div key={item.id} className="pt-3 text-base font-bold">
                    <p>Load {item.luwa_3A_kW} kW</p>
                  </div>
                );
              })}
              <p className="text-xs text-muted-foreground">
        ...
        <Card className="p-0">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 p-4">
            <CardTitle className="text-xl font-bold">LUWA 3B</CardTitle>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="none"
          ...
          <CardContent className="flex justify-evenly">
            ...
            >
              ...
              >
                {data.map((item) => item.luwa_3B_kW.toFixed(0))} kW
              </div>
              <canvas id="luwa3B" width="100" height="100" />
            </div>
            <div className="">
              {data.map((item) => {
                return (
                  <div key={item.id} className="pt-3 text-base font-bold">
                    <p>Load {item.luwa_3B_kW} kW</p>
                  </div>
                );
              })}
              <p className="text-xs text-muted-foreground">
