...
function Row({
  ...
  return (
    ...
    >
      <span
        className={`text-sm ${emphasize ? "font-semibold text-gray-900" : "text-gray-600"}`}
      >
        {label}
      </span>
      <span
...
export default function SteamGenerationReport() {
  ...
  /** ---------- Power House 3 totals ---------- */
  const whrbDiffs3 = FREE_STEAM_3_ORDERED.map((id) => diffFor(id));
  const freeSteamTotal3 = whrbDiffs3.reduce<number>(
    (sum, v) => sum + (v ?? 0),
    0,
  );
  const grandTotal3 = freeSteamTotal3;
  ...
  /** ---------- Power House 4 totals ---------- */
  const whrbDiffs4 = FREE_STEAM_4_ORDERED.map((id) => diffFor(id));
  const freeSteamTotal4 = whrbDiffs4.reduce<number>(
    (sum, v) => sum + (v ?? 0),
    0,
  );
  const grandTotal4 = freeSteamTotal4;
  ...
  const gasAmountRsM3 =
    gasUsedM3 != null &&
    gasCostPerM3 !== "" &&
    Number.isFinite(gasCostPerM3 as number)
      ? gasUsedM3 * (gasCostPerM3 as number)
      : null;
  ...
  const hrsgGasUsedM3 = diffFor(HRSG_GAS_TOTALIZER_ID);
  const hrsgDuctTons =
    hrsgGasUsedM3 != null ? hrsgGasUsedM3 / HRSG_DUCT_DIVISOR : 0;
  const hrsgGasUsedMMBtu =
    hrsgGasUsedM3 != null ? hrsgGasUsedM3 * MMBTU_PER_M3 : null;
  const hrsgGasAmountRs =
    hrsgGasUsedM3 != null &&
    gasCostPerM3 !== "" &&
    Number.isFinite(gasCostPerM3 as number)
  ...
  const coalConsumedDiff = diffFor(COAL_CONSUMED_ID);
  const coalSteamDiff = diffFor(COAL_STEAM_ID);
  const coalConsumedDisplay =
    coalConsumedDiff == null ? null : coalConsumedDiff / 1000; // kg -> ton
  const coalAmount =
    coalConsumedDisplay != null &&
    coalCostPerTon !== "" &&
    Number.isFinite(coalCostPerTon as number)
      ? coalConsumedDisplay * (coalCostPerTon as number)
      : null;
  const costPerTonSteam =
    coalAmount != null && coalSteamDiff != null && coalSteamDiff > 0
      ? coalAmount / coalSteamDiff
      : null;
  ...
  function fmtPct(part: number | null | undefined, total: number) {
    if (part == null || Number.isNaN(part) || total <= 0) return "—";
    return (
      ((part / total) * 100).toLocaleString("en-US", {
  ...
  function exportToXlsx({
    ...
    const HRSG_GAS_MMBTU = HRSG_GAS_M3 * MMBTU_PER_M3;
    const HRSG_GAS_AMOUNT_RS = HRSG_GAS_M3 * Number(gasCostPerM3 || 0);
    const HRSG_COST_RS_PER_TON =
      HRSG_DUCT_TON > 0 ? HRSG_GAS_AMOUNT_RS / HRSG_DUCT_TON : 0;
    ...
    const GAS_MMBTU_TOTAL = HRSG_GAS_MMBTU + GAS_MMBTU;
    const GAS_AMOUNT_RS_TOTAL = HRSG_GAS_AMOUNT_RS + GAS_AMOUNT_RS;
    const GAS_COST_RS_PER_TON_TOTAL =
      GAS_TON_TOTAL > 0 ? GAS_AMOUNT_RS_TOTAL / GAS_TON_TOTAL : 0;

    // ===== COAL (outsourced boiler)
    const COAL_TON_STEAM = d(177);
    const COAL_CONS_TON = d(4649) / 1000; // kg → ton
    const COAL_AMOUNT_RS = COAL_CONS_TON * Number(coalCostPerTon || 0);
    const COAL_COST_RS_PER_TON =
      COAL_TON_STEAM > 0 ? COAL_AMOUNT_RS / COAL_TON_STEAM : 0;
    ...
    const endDt = new Date(end);
    const days = Math.max(0, Math.round((+endDt - +startDt) / 86_400_000));
    const monthHdr = new Intl.DateTimeFormat("en-GB", {
      month: "long",
      year: "numeric",
    })
      .format(startDt)
    ...
    set(7, 1, "FREE");
    set(7, 3, "GENERATION");
    ["TURBINE", "WHRB 1", "WHRB 2", "WHRB 3", "WHRB 4", "TOTAL"].forEach(
      (h, i) => set(8, 3 + i, h),
    );
    set(9, 1, "UNITS");
    set(9, 2, "tons");
    [FREE_TURBINE, WHRB1, WHRB2, WHRB3, WHRB4, FREE_TOTAL].forEach((v, i) =>
      set(9, 3 + i, v),
    );
    ...
    set(12, 13, {
      t: "n",
      f: `IF(${A(12, 12)}>0,${A(12, 12)}/${A(15, 12)},0)`,
    });
    set(13, 11, "GAS");
    set(13, 12, GAS_GEN);
    set(13, 13, {
      t: "n",
      f: `IF(${A(13, 12)}>=0,${A(13, 12)}/${A(15, 12)},0)`,
    });
    set(14, 11, "COAL");
    set(14, 12, COAL_GEN);
    set(14, 13, {
      t: "n",
      f: `IF(${A(14, 12)}>=0,${A(14, 12)}/${A(15, 12)},0)`,
    });
    set(15, 11, "TOTAL");
    set(15, 12, TOTAL_GEN);
    set(15, 13, 1);
    ...
    set(20, 13, {
      t: "n",
      f: `IF(${A(23, 12)}>0,${A(20, 12)}/${A(23, 12)},0)`,
    });
    set(21, 11, "GAS");
    set(21, 12, GAS_AMOUNT_RS);
    set(21, 13, {
      t: "n",
      f: `IF(${A(23, 12)}>0,${A(21, 12)}/${A(23, 12)},0)`,
    });
    set(22, 11, "HRSG");
    set(22, 12, HRSG_GAS_AMOUNT_RS);
    set(22, 13, {
      t: "n",
      f: `IF(${A(23, 12)}>0,${A(22, 12)}/${A(23, 12)},0)`,
    });
    set(23, 11, "COAL");
    set(23, 12, COAL_AMOUNT_RS);
    set(23, 13, {
      t: "n",
      f: `IF(${A(23, 12)}>0,${A(23, 12)}/${A(23, 12)},0)`,
    });
    set(24, 11, "TOTAL");
    set(24, 12, {
      t: "n",
      f: `${A(20, 12)}+${A(21, 12)}+${A(22, 12)}+${A(23, 12)}`,
    });
    set(24, 13, 1);
  ...
  const handleExportXlsx = () => {
    // const diffs = buildDiffMap(data)
    const diffs: Record<number, number> = {};
    const allTagIds = new Set(data.map((d) => d.tagId));
    ...
    exportToXlsx({
      ...
      diffs,
      gasCostPerM3:
        typeof gasCostPerM3 === "number"
          ? gasCostPerM3
          : gasCostPerM3 === ""
            ? 0
            : gasCostPerM3,
      gasCostPerMMBtu:
        typeof gasCostPerMMBtu === "number"
          ? gasCostPerMMBtu
          : gasCostPerMMBtu === ""
            ? 0
            : gasCostPerMMBtu,
      coalCostPerTon:
        typeof coalCostPerTon === "number"
          ? coalCostPerTon
          : coalCostPerTon === ""
            ? 0
            : coalCostPerTon,
          ...
          <>
            ...
            <SectionCard title="Power House 1" className="mb-6">
              ...
              <div className="md:hidden space-y-4">
                ...
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                  <Row
                    label="FREE STEAM TOTAL"
                    value={freeSteamTotal1}
                    emphasize
                  />
                </div>
                <div className="bg-purple-50 rounded-lg p-4 border border-purple-300">
            ...
            <SectionCard title="Coal Boiler (Outsource)" className="mb-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-amber-50 rounded-lg p-4 border border-amber-200">
                  <div className="text-xs text-gray-700 uppercase tracking-wide mb-1 font-medium">
                    Total Cost
                  </div>
                  <div className="text-xl font-bold text-gray-900 font-mono tabular-nums">
                    {fmt(coalAmount)}
            ...
            <SectionCard title="HRSG Steam Breakdown" className="mb-6">
              ...
              <div className="mt-3 text-xs text-gray-500 italic">
                HRSG Steam Totalizer = Free Turbine + HRSG Duct
              </div>
            </SectionCard>
          </>
        )}
        ...
        {data.length === 0 && !loading && (
          <div className="text-center py-12 bg-white rounded-lg border border-gray-200 shadow-sm">
            ...
            <p className="text-gray-500 text-sm">
              No data available. Please fetch a report to view results.
            </p>
          </div>
        )}
      </div>
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  value={gasCostPerMMBtu}
                  onChange={(e) =>
                    setGasCostPerMMBtu(asNumberOrEmpty(e.target.value))
                  }
                  placeholder="Enter cost per MMBtu"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                />
              ...
              <div>
                ...
                <input
                  ...
                  value={gasCostPerM3}
                  onChange={(e) =>
                    setGasCostPerM3(asNumberOrEmpty(e.target.value))
                  }
                  placeholder="Enter cost per m³"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                />
              ...
              <div>
                ...
                <input
                  ...
                  value={coalCostPerTon}
                  onChange={(e) =>
                    setCoalCostPerTon(asNumberOrEmpty(e.target.value))
                  }
                  placeholder="Enter cost per ton"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                />
              ...
              <div>
                ...
                <input
                  ...
                  value={dieselCostPerLiter}
                  onChange={(e) =>
                    setDieselCostPerLiter(asNumberOrEmpty(e.target.value))
                  }
                  placeholder="Enter cost per liter"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                />
